...
test('SemanticStore upserts and deletes preference vectors', async () => {
  const dir = await mkdtemp(path.join(os.tmpdir(), 'ft-semantic-store-'));
  const store = await SemanticStore.open(path.join(dir, 'semantic.lance'));
  ...
  try {
    await store.upsertPreferences([
      {
        id: 'like:prefer:topic:ai agents',
        action: 'like',
        disposition: 'prefer',
        rawText: 'AI Agents',
        normalizedText: 'ai agents',
        textHash: 'pref-a',
        embeddingVersion: 'aliyun-bailian:text-embedding-v4',
        vector: [1, 0],
      },
      {
        id: 'bookmark:avoid:topic:sports',
        action: 'bookmark',
        disposition: 'avoid',
        rawText: 'sports',
        normalizedText: 'sports',
        textHash: 'pref-b',
        embeddingVersion: 'aliyun-bailian:text-embedding-v4',
        vector: [0, 1],
      },
    ...
    const hits = await store.searchPreferences([0.9, 0.1], 'like', 'prefer', 3);
    assert.equal(hits[0]?.id, 'like:prefer:topic:ai agents');
    assert.equal(await store.countPreferences('bookmark', 'avoid'), 1);

    await store.deletePreferenceIds(['bookmark:avoid:topic:sports']);
    assert.equal(await store.countPreferences('bookmark', 'avoid'), 0);
  } finally {
    await store.close();
    await rm(dir, { recursive: true, force: true });
  }
