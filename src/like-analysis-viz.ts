import { readLikeAnalysisRecords, getLikeAnalysisStatus } from './like-analysis-store.js';
import type { LikeAnalysisRecord } from './like-analysis-types.js';
...
export interface LikeAnalysisVizOptions {
  limitTags?: number;
  width?: number;
}
...
function countBy(records: LikeAnalysisRecord[], selector: (record: LikeAnalysisRecord) => string): CountEntry[] {
...
function tagCounts(records: LikeAnalysisRecord[]): CountEntry[] {
  const counts = new Map<string, number>();
  for (const record of records) {
...
function renderMatrix(records: LikeAnalysisRecord[], categories: CountEntry[], contentTypes: CountEntry[]): string[] {
  ...
  if (visibleCategories.length === 0 || visibleTypes.length === 0) return ['  no classified likes'];
...
function summarizeConfidence(records: LikeAnalysisRecord[]): string {
...
export async function renderLikeAnalysisViz(options: LikeAnalysisVizOptions = {}): Promise<string> {
  ...
  const lines: string[] = [];
  lines.push(`${C.title}${BOLD}Like Classification${RESET}`);
  ...
  lines.push('');
  lines.push(`${DIM}Try: ft likes classify list --category ${categoryCounts[0]?.name ?? 'ai'}`);
  lines.push(`     ft likes classify list --content-type ${contentTypeCounts[0]?.name ?? 'repo'}`);
  lines.push(`     ft likes classify list --tag ${tags[0]?.name ?? 'open-source'}${RESET}`);
